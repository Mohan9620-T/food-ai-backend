...
def _headers(client, email="plans@example.com"):
    client.post(
        "/users/", json={"fullname": "Plan User", "email": email, "password": "plan-password"}
    )
        ...
        "days": [
            {
                ...
                "meals": [
                    {
                        "meal_slot": slot,
                        "description": f"{slot} meal",
                        "items": [
                            {
                                "food_name": "oatmeal"
                                if not (day == 0 and slot == "snack")
                                else "mystery food",
                                "quantity": 100,
                                "unit": "g",
                            }
...
def test_generate_grounded_plan_prompt_list_get_delete(client, monkeypatch):
    ...
    def lookup(self, item):
        if item.food_name == "mystery food":
            return NutritionResult(item.food_name, item.quantity, item.unit)
    ...
    unmatched = next(
        item for meal in plan["meals"] for item in meal["items"] if item["fdc_id"] is None
    )
...
def test_malformed_json_retries_once_then_fails(client, monkeypatch):
    headers = _headers(client, "bad-plan@example.com")
    _profile(client, headers)
    attempts = 0
