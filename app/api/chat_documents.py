...
@router.post("", response_model=ChatDocumentResponse, summary="Upload a chat document")
async def upload_document(
    ...
    if expected_type is None or content_type not in {expected_type, "application/octet-stream"}:
    ...
    if session_id is None:
        ...
        existing_session = repository.get_session(db, session_id, user_id)
        if existing_session is None:
            raise HTTPException(status_code=404, detail="Chat session not found")
        resolved_session = existing_session
...
@router.post("/generate", response_model=ChatDocumentResponse, summary="Generate a chat document")
async def generate_document(
    ...
    records = repository.get_message_history(db, resolved_session_id)
    ...
    try:
        content = await asyncio.to_thread(service.generate_content, payload.instruction, repository.get_document_summaries(db, resolved_session_id), history, profile)
