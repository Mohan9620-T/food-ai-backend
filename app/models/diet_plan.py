...
class DietPlan(Base):
    ...
    meals = relationship(
        ...
        order_by="DietPlanMeal.day_of_week, DietPlanMeal.id",
    )
...
class DietPlanMealItem(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    diet_plan_meal_id = Column(
        Integer, ForeignKey("diet_plan_meals.id"), nullable=False, index=True
    )
